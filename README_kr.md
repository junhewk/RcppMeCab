...
리눅스, macOS의 `ja`와 `ko` 프로필에서는 MeCab이 이미 시스템에 설치되어 있는 경우 (`mecab-config`으로 감지) 시스템 엔진을 우선 사용합니다. `zh` 프로필은 컴파일러와 mecab-jieba 사전의 호환성을 위해 항상 고정된 표준 MeCab 엔진을 소스에서 빌드합니다. `MECAB_LANG`은 설치 시 설정이며 실행 중인 R 세션에서 엔진을 전환하지 않습니다.
...
RcppMeCab이 자체 엔진을 빌드할 때 해당 MeCab 사전이 **자동으로 다운로드 및 설치**됩니다:
...
번들 사전은 패키지의 `dic/` 디렉토리에 저장되며 자동으로 사용됩니다. `ja`와 `ko` 프로필이 시스템 MeCab을 재사용하는 경우에는 시스템에 설정된 사전을 사용합니다. `zh` 프로필은 고정된 엔진과 함께 mecab-jieba를 항상 컴파일하여 번들로 포함합니다.
